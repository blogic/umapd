...
import proto_higherlayer from 'umap.proto.higherlayer';
	...
	try {
		const handled = msg.run_handler()
			...
			|| proto_policy.handle_cmdu(i1905lif, dstmac, srcmac, msg)
        		|| proto_higherlayer.handle_cmdu(i1905lif, dstmac, srcmac, msg)
		        ;

		if (!handled)
			log.warn(`Not handling CMDU [${msg.mid}] ${utils.cmdu_type_ntoa(msg.type)}`);
	} catch(e) {
		log.error(e.stacktrace[0].context);
	}
