...
const IProtoCapabilities = {
	...
	query_client_capability: function (req) {
		...
		query.on_reply(response => {
			...
			const client_capability = response.get_tlvs(defs.TLV_CLIENT_CAPABILITY_REPORT);
			...
			if (!client_capability.result_code)
				ret.farme_body = client_capability.frame_body;
	...
	handle_cmdu: function (i1905lif, dstmac, srcmac, msg) {
		...
		if (msg.type === defs.MSG_AP_CAPABILITY_QUERY) {
			...
			const station = ubusclient.call('rrm', 'station', { bssid: client_info.bssid, macaddr: client_info.mac_address });
