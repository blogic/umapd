...
const IProtoChannelSelection = {
	...
	emit_channel_preference_query: function(req) {
		if (!req.args.al_address || req.args.al_address == model.address)
			return req.reply(null, defs.UBUS.STATUS_NOT_FOUND);
	
		const i1905dev = model.lookupDevice(req.args.al_address);
		if (!i1905dev)
			return req.reply(null, defs.UBUS.STATUS_NOT_FOUND);
	...
	emit_channel_selection_request: function(req) {
		if (!req.args.al_address || req.args.al_address == model.address || !req.args.ruid)
			return req.reply(null, defs.UBUS.STATUS_NOT_FOUND);
