...
const IProtoScanning = {
	...
	initiate_scan: function (req) {
		...
		if (req.args.macaddress) {
			...
			if (!i1905dev)
				return req.reply(null, ubus.const.STATUS_NOT_FOUND); /* device not found */

			for (let ruid in req.args.radios)
				if ((ap_capas[ruid] = dev.getBasicAPCapability(ruid)) == null)
					return req.reply(null, ubus.const.STATUS_NOT_FOUND); /* radio on device not found */
		}
		...
		/* device address omitted, lookup by radios */
			...
			for (let ruid in req.args.radios) {
				...
				if (!rv)
					return req.reply(null, ubus.const.STATUS_NOT_FOUND); /* no device with specified ruid found */

				i1905dev ??= rv[0];

				if (rv[0] !== i1905dev)
					return req.reply(null, ubus.const.STATUS_INVALID_ARGUMENT); /* radios refer to different devices */

				ap_capas[ruid] = rv[1];
			}

			if (!i1905dev)
				return req.reply(null, ubus.const.STATUS_INVALID_ARGUMENT); /* neither address nor radio given */
		}
		...
		if (!length(ap_capas)) {
			...
			if (!length(ap_capas))
				return req.reply(null, ubus.const.STATUS_NO_DATA); /* device has no radios */
		}
