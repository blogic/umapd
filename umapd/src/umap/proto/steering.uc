...
let IFailedConnections = {
	...
	init: function () {
		events.register('wireless.association-error', this.emit_failed_connection_message);
		events.register('policy.unsuccessful-association', this.update_policy);
	},

	update_policy: function(data) {
		log.debug('received a failed connection policy update');
		this.policy = data;

		if (this.policy.report_unsuccessful_assocs && this.policy.max_reporting_rate) {
			const self = this;
			this.interval ??= interval(FAILED_CONNECTION_INTERVAL, () => { self.failed_connections = 0; }); 
	...
	emit_failed_connection_message: function(event) {
		if (!this.policy.report_unsuccessful_assocs)
			return;

		this.failed_connections++;
		if (this.policy.max_reporting_rate && this.failed_connections > this.policy.max_reporting_rate)
			return;
